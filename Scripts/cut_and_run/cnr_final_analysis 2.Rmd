...
# genebody counts
#test <- readRDS("../../../cut_and_run/bg_GeneBodies_all (1).bam_allchr_E6Xist")
test <- readRDS("./ProcessedData/cut_and_run/bg_GeneBodies_all (1).bam_allchr_E6Xist")
test
...
# promoter counts
test <- readRDS("./ProcessedData/cut_and_run/bg_promoters_-500+1000_all (1).bam_allchr_E6Xist")
test
...
deseq_sizefactors_ubi <- readRDS("./ProcessedData/cut_and_run/deseq_ubi_total.rds") %>% colData() %>% data.frame() %>% pull(sizeFactor, name = Condition)
deseq_sizefactors_met <- readRDS("./ProcessedData/cut_and_run/deseq_met_total.rds") %>% colData() %>% data.frame() %>% pull(sizeFactor, name = Condition)

sfdf_ubi <- readRDS("./ProcessedData/cut_and_run/deseq_ubi_total.rds") %>% colData() %>% data.frame() %>% dplyr::select(c("bam.files", "HistoneMark", "Condition", "Replicate", "sizeFactor")) %>% mutate(bam.files = basename(bam.files)) %>% rownames_to_column("Sample") %>% column_to_rownames("bam.files")
sfdf_met <- readRDS("./ProcessedData/cut_and_run/deseq_met_total.rds") %>% colData() %>% data.frame() %>% dplyr::select(c("bam.files", "HistoneMark", "Condition", "Replicate", "sizeFactor")) %>% mutate(bam.files = basename(bam.files)) %>% rownames_to_column("Sample") %>% column_to_rownames("bam.files")

metadata_1 <- readRDS("./ProcessedData/cut_and_run/deseq_ubi_total.rds") %>% colData() %>% data.frame() %>% dplyr::select(c("bam.files", "HistoneMark", "Condition", "Replicate", "sizeFactor"))
metadata_2 <- readRDS("./ProcessedData/cut_and_run/deseq_met_total.rds") %>% colData() %>% data.frame() %>% dplyr::select(c("bam.files", "HistoneMark", "Condition", "Replicate", "sizeFactor"))
...
rownames(data_ref) <- rowData(data_promoters)$gene_name
...
rownames(data_alt) <- rowData(data_promoters)$gene_name
...
data_long_x %>%
  ...
  dplyr::filter(Context != "Genebody, Promoter") %>%
...
# data_long_x %>%
#   dplyr::filter(Condition %in% c("NoDox", "21dDox")) %>%
#   # dplyr::filter(Overlap != c("Genebody, Promoter")) %>%
#   dplyr::select(-c(size_factor, value, fold_change)) %>%
#   mutate(value_norm = log2(value_norm)) %>%
#   pivot_wider(names_from = Overlap, values_from = value_norm) %>%
#   ggplot(aes(x = Xa, y = Xi, col = Condition)) + 
#     geom_density_2d() + 
#     facet_wrap(~HistoneMark + Replicate)
# ggsave("./Plots/latest_figure/gains_overall_ubi.pdf", width = 12, height = 12)

data_long_x %>%
  dplyr::filter(HistoneMark == "H2AK119ubi") %>%
  group_by(Symbol, Condition, HistoneMark, Allele, Context) %>%
  summarize(value_norm = mean(value_norm)) %>%
  ggplot(aes(x = Condition, fill = Allele)) + 
...
data_long_x %>%
 dplyr::filter(HistoneMark == "H3K27me3") %>%
  ...
  ggplot(aes(x = Condition, fill = Allele)) + 
    ...
    scale_fill_manual(values = c("black", "red")) + 
    facet_wrap(~HistoneMark + Context, nrow = 1) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + 
    theme_paper() + labs(fill = "Sample") + ylab("Normalized Counts") + ylim(c(0, 400))
...
#   add_column(Allele = "Xa")
# 
# data_long <- rbind(data_ref, data_alt) %>% 
#   mutate(size_factor = rbind(sfdf_ubi, sfdf_met)[paste0("E6", Condition, Replicate, "_", HistoneMark, "_Gall_sorted.bam"), ]$sizeFactor) %>%
#   mutate(value_norm = value / size_factor) %>%
#   mutate(Condition = ifelse(Condition == "21dD", "21dDox", Condition)) %>%# one condition is labelled as 21dDox, one as 21dD, correct this here
#   rename(Gene = "Symbol") %>%
#   right_join(gene_annotation[ , c("escape_status", "gene_category", "Symbol")])
#   
# rm(data_ref, data_alt)
...
data_long %>%
  ...
  ggplot(aes(x = Condition, col = gene_category)) + 
    ...
    scale_color_manual(values = c("grey", "darkgreen")) + xlab("")
...
data_long %>%
  ...
  arrange(-FC_wo) %>% 
  ggplot(aes(x = FC_dox, y = FC_wo, col = gene_category)) + geom_point() +
    ggrepel::geom_text_repel(aes(label = Symbol)) + geom_abline() + theme_paper() + 
...
ggsave("./Plots/latest_figure/gains_reversible_irreversible_scatter_me3.pdf", width = 20, height = 12)
