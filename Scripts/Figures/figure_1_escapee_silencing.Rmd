...
This script reproduces most plots for Figure 1 and associated supplementary figures (S1-S4) in the Xist paper. Preprocessing of raw RNA sequencing data is done in ./Mapping/..., that yields allele-specific expression counts which are combined into a SingleCellExperiment (SCE) object in ./Preprocessing/generate_sce.R.
...
Now, read in the SCE, remove non-X genes and exclude lowly expressed genes, and samples unnecessary for this analysis: 
...
counts(data_upload) %>% rownames_to_column("Gene") %>% write_delim(., "./ProcessedData/geo_main/main_celllines_total_counts.txt", delim = "\t")
counts_active(data_upload) %>% rownames_to_column("Gene") %>% write_delim(., "./ProcessedData/geo_main/main_celllines_genome1_counts.txt", delim = "\t")
counts_inactive(data_upload) %>% rownames_to_column("Gene") %>% write_delim(., "./ProcessedData/geo_main/main_celllines_genome2_counts.txt", delim = "\t")
...
# Export expression level table for secondary analysis: 
data.frame(
...
ratios %>%
  ...
  add_column(position = start(rowRanges(data_here[as.character(.$name), ]))) -> df_plot_s1f
...
ratios %>%
  ...
  add_column(escape_group = factor(rowData(data[.$name, ])$EscapeAnnotation, levels = c("Constitutive", "Facultative", "NPC-specific"))) -> df_plot_s1g
...
ratios %>%
  ...
  group_by(name) %>%
  mutate(escape_in_clones = n() >= 11) %>%
  add_column(escape_group = factor(rowData(data[.$name, ])$EscapeAnnotation, levels = c("Constitutive", "Facultative", "NPC-specific"))) -> df_plot_s1i
...
ratios %>%
  ...
  mutate(Condition = factor(Condition, levels = rev(c("Control", "Dox (3d)", "Dox (7d)", "Dox (14d)", "Dox (21d)")))) -> df_plot_s2a

df_plot_s2a %>%
  ggplot(aes(reorder(name, position), y = interaction(Clone, Condition, sep = " -- "), group = Clone, fill = value, col = value)) + 
  geom_tile(width = 0.9) +
    scale_fill_gradient2(midpoint = 0.5, mid = "#117733", low = "#BEBEBE", high = "black", limits = c(0, 1), na.value = "white") + 
...
ratios[e6_escapees, ] %>%
  ...
  dplyr::filter(!is.na(escape_status)) -> df_plot_1g
...
ratios[e6_escapees, ] %>%
  ...
  dplyr::select(-c("values3", "values6", "paired")) -> pval_results
...
ratios[e6_escapees, ] %>%
  ...
  add_column(escape_status = unlist(rowData(data_here)[match(.$name, rownames(data_here)), ]$EscapeAnnotation)) %>%
  mutate(escape_status = factor(escape_status, c("NPC-specific", "Facultative", "Constitutive"))) %>%
  dplyr::filter(!is.na(escape_status)) -> df_plot_s2b
...
ratios[e6_escapees, ] %>%
  ...
  ungroup() %>%
  dplyr::filter(!duplicated(pvals)) %>%
  mutate(padj = p.adjust(pvals, method = "BH")) %>% 
  dplyr::select(-c("values2", "values4", "paired")) -> pval_results
...
ratios %>%
  ...
  #dplyr::filter(value < 0.7) %>%
  #dplyr::filter(Clone == "E6") %>%
  mutate(Condition = factor(Condition, levels = rev(c("Control", "Dox (3d)", "Dox (7d)", "Dox (14d)", "Dox (21d)")))) -> df_plot_s2c
...
ratios %>%
  ...
  group_by(Clone) %>%
  dplyr::filter(name %in% escapees_cl30s[[unique(Clone)]]) -> df_plot_s2f
...
ratios %>%
  ...
  mutate(Clone2 = ifelse(grepl("CL30", Clone), "CL30", "CL31")) -> df_plot_s2d
...
df_plot_s2d %>%
  ggplot(aes(x = value, y = factor(Condition, levels = rev(levels(Condition))), fill = Clone)) +
    ...
    theme_paper() + xlab("Allelic Ratio (Xi / (Xi + Xa))") +
    theme(axis.text.x=element_text(angle = 45, hjust = 1)) +
    labs(fill = "Clone") + ylab("") + facet_wrap(~Clone2)
...
rbind(df_inactive, rbind(df_active, df_total)) -> df_plot_s4a
df_plot_s4a %>%
...
rbind(df_inactive, rbind(df_active, df_total)) -> df_plot_s4b
...
merged_df -> df_plot_1i -> df_plot_1k
...
plot_decay_gene <- function(data_here, all_coefs, all_coefs_offset, gene, save_plot = F, path = "./", return_data = F){
  ...
  data.frame(
    ...
    timepoint = as.numeric(data_here$ndDox), 
    experiment = data_here$Experiment
  ) %>% mutate(rates = inactive / (inactive + active)) -> data_to_plot
...
plot_decay_gene(data_here_e6_plot, merged_df_e6, merged_df_e6, "Mecp2", save_plot = T)
plot_decay_gene(data_here_e6_plot, merged_df_e6, merged_df_e6, "Pbdc1", save_plot = T)

df_plot_s3b_1 <- plot_decay_gene(data_here_e6_plot, merged_df_e6, merged_df_e6, "Mecp2", return_data = T)
df_plot_s3b_2 <- plot_decay_gene(data_here_e6_plot, merged_df_e6, merged_df_e6, "Pbdc1", return_data = T)
df_plot_s3b <- rbind(cbind(df_plot_s3b_1, gene = "Mecp2"), cbind(df_plot_s3b_2, gene = "Pbdc1"))
...
data.frame(Gene = e6_escapees,
  ...
  mutate(Clusterings = ifelse(is.na(Clusterings), "Singleton", Clusterings)) %>% rename(Gene = "name") -> df_plot_1f_2

df_plot_1f <- df_plot_1f_1 %>% right_join(df_plot_1f_2, by = "name")
...
df_plot_1e <- df_plot_1e_pre %>% full_join(df_plot_1f_2, by = "name")
...
comb_results %>% 
  mutate(condition = ifelse(condition == "inter", "Random genes", "Adjacent genes")) -> df_plot_s3e
...
source_data_1 <- list(df_plot_1d, df_plot_1e, df_plot_1f, df_plot_1g, df_plot_1i, df_plot_1j, df_plot_1k, 
                      df_plot_s1f, df_plot_s1g, df_plot_s1i, # 8, 9, 10
                      df_plot_s2a, df_plot_s2b, df_plot_s2c, df_plot_s2d, df_plot_s2e, df_plot_s2f, df_plot_s2g, df_plot_s2g, df_plot_s2i, # 11 - 19
                      df_plot_s3a, df_plot_s3b, df_plot_s3c, df_plot_s3c, df_plot_s3e, df_plot_s3f, # 20 - 25
                      df_plot_s4a, df_plot_s4b, df_plot_s4c, df_plot_s4d) # 26 - 29
saveRDS(source_data_1, "./ProcessedData/source_data/source_data_1.rds")
