...
counts(data) %>% rownames_to_column("Gene") %>% write_delim("../ProcessedData/geo_astrocytes/astrocytes_total_counts.csv", delim = "\t")
counts_active(data) %>% rownames_to_column("Gene") %>% write_delim("../ProcessedData/geo_astrocytes/astrocytes_active_counts.csv", delim = "\t")
counts_inactive(data) %>% rownames_to_column("Gene") %>% write_delim("../ProcessedData/geo_astrocytes/astrocytes_inactive_counts.csv", delim = "\t")
...
ratios[consistent_escapees, ] %>%
  ...
  mutate(Condition = factor(Condition, levels = (c("Astro_no_dox3", "Astro_dox3", "Astro_no_dox7", "Astro_dox7")))) %>%
  ggplot(aes(x = Condition, y = value, fill = Replicate)) + 
    geom_boxplot(col = "black", outlier.color = "grey", position = position_dodge2(preserve = "single")) + 
    theme_paper() + ylab("ASE (d-score)") + 
    theme(axis.text.x=element_text(angle = 45, hjust = 1)) + 
...
sim_data %>% 
  ...
  mutate(group = factor(group, levels = c("NPC (Real data)", "Astrocyte (Real data)", "Simulated"))) %>%
