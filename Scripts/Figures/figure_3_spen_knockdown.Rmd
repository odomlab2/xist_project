...
xist_data_here %>%
  mutate(Clone = str_extract(Clone, "CL30|CL31")) %>%
...
#   ggplot(aes(x = Condition2, y = value, fill = Experiment, col = Clone)) + geom_boxplot() + 
#     theme_paper() + ylab("ASE (d-score)") + 
#     theme(axis.text.x=element_text(angle = 45, hjust = 1)) + 
#     facet_wrap(~Timepoint, scale = "free_x") + 
#     xlab("") + theme_paper() + scale_color_manual(values = c("grey", "black"))
...
ratios %>%
  ...
  group_by(Clone) %>%
  dplyr::filter(name %in% escapees_per_clone[[unique(Clone)]]) %>%
  # dplyr::filter(Condition %in% c("Control", "Aux (2d)")) %>%
  ungroup() %>%
  add_column(Timepoint = pmax(.$Dox, .$Aux)) %>%
  add_column(Condition2 = paste0(str_extract(.$Condition, "Aux"), "_", str_extract(.$Condition, "Dox"))) %>%
  add_column(TimeSeries = ifelse(.$Dox == 0, list(c("3", "7")), .$Dox)) %>%
...
#     scale_color_manual(values = escape_colors) + theme_paper() + 
#     geom_smooth(method = "lm", linetype = 'dashed', color = "darkblue") + 
#     xlab("Control (Allelic Ratio)") + ylab("3d Dox (Allelic Ratio)") + 
#     labs(col = "Escape Category") + 
#     theme(legend.position = "None") -> p1
# # ggplot2::ggsave("./Plots/Fig2/scatter_control_vs_3d_dox.pdf", width = 7, height = 5)
# 
# data.frame(
#   Gene = rownames(ratios_escapees), 
#   Control = rowMeans(ratios_escapees[,data_here$ConditionClean == "Control"]), 
...
#     xlab("Control (Allelic Ratio)") + ylab("3d Dox + Aux (Allelic Ratio)")  + 
#     labs(col = "Escape Category") + 
#     theme(legend.position = "None") -> p2
# # ggplot2::ggsave("./Plots/Fig2/scatter_control_vs_3d_dox_aux.pdf", width = 7, height = 5)
# 
# data.frame(
#   Gene = rownames(ratios_escapees), 
#   Control = rowMeans(ratios_escapees[,data_here$ConditionClean == "Control"]), 
...
#     scale_color_manual(values = escape_colors) + theme_paper() + geom_smooth(method = "lm", linetype = 'dashed', color = "darkblue") + 
#     xlab("Control (Allelic Ratio)") + ylab("7d Dox (Allelic Ratio)") + 
#     labs(col = "Escape Category") + 
#     theme(legend.position = "None") -> p3
# # ggplot2::ggsave("./Plots/Fig2/scatter_control_vs_7d_dox.pdf", width = 7, height = 5)
# 
# data.frame(
#   Gene = rownames(ratios_escapees), 
#   Control = rowMeans(ratios_escapees[,data_here$ConditionClean == "Control"]), 
...
#     scale_color_manual(values = escape_colors) + theme_paper() + geom_smooth(method = "lm", linetype = 'dashed', color = "darkblue") + 
#     xlab("Control (Allelic Ratio)") + ylab("7d Dox + Aux (Allelic Ratio)") + 
#     labs(col = "Escape Category") + 
#     theme(legend.position = "None") -> p4
# # ggplot2::ggsave("./Plots/Fig2/scatter_control_vs_7d_dox_aux.pdf", width = 7, height = 5)
...
data.frame(
  ...
  escape_category = rowData(data_here[rownames(ratios_escapees), ])$EscapeAnnotation
) %>% make_plot_here(., x_label = "Control (Allelic Ratio)", y_label = "3d Dox + Aux (Allelic Ratio)") -> p_here
p_here
...
data.frame(
  ...
  escape_category = rowData(data_here[rownames(ratios_escapees), ])$EscapeAnnotation
) %>% make_plot_here(., x_label = "Control (Allelic Ratio)", y_label = "7d Dox + Aux (Allelic Ratio)") -> p_here
p_here
...
# data_pca <- data_with_autosomes[,data_with_autosomes$Condition %in% c("Aux_0_Dox_0_WO_0_WOAuxNO", "Aux_0_Dox_7_WO_0_WOAuxNO")]
# data_pca <- computeSumFactors(data_pca)
# data_pca <- logNormCounts(data_pca)
# data_pca <- runPCA(data_pca)
# rotations_pca <- attr(calculatePCA(data_pca), "rotation")
# 
# (counts_active(data_pca) / (counts_active(data_pca) + counts_inactive(data_pca))) %>%
#   add_column("chr" = as.character(seqnames(rowRanges(data_pca)))) %>%
#   add_column("pos" = start(rowRanges(data_pca))) %>%
#   pivot_longer(-c("chr", "pos")) %>% 
#   add_column(Clone = colData(data_pca)[.$name, ]$Clone) %>%
#   # dplyr::filter(chr %in% c(1, 11, 16, 13, 6)) %>%
#   add_column(Condition = colData(data_pca)[.$name, ]$ConditionClean) %>%
#   add_column(Replicate = colData(data_pca)[.$name, ]$Replicate) %>%
#   ggplot(aes(x = chr, y = value, col = Condition)) + stat_summary() + facet_wrap(~Clone+Replicate) + ylim(c(0, 1)) + geom_hline(yintercept = .5)
# ggsave("./Plots/FigsX_autosomal_joint_analysis/allelic_ratios_autosome.pdf", width = 20, height = 20)
# 
# (counts_active(data_pca) / (counts_active(data_pca) + counts_inactive(data_pca))) %>%
#   add_column("chr" = as.character(seqnames(rowRanges(data_pca)))) %>%
#   add_column("pos" = start(rowRanges(data_pca))) %>%
...
#   add_column(Replicate = colData(data_pca)[.$name, ]$Replicate) %>%
#   dplyr::filter(chr == 11) %>%
#   ggplot(aes(x = pos, y = value, col = Condition)) + geom_point(size = .1) + facet_wrap(~Clone+Replicate) + ylim(c(0, 1)) + geom_hline(yintercept = .5)
# ggsave("./Plots/FigsX_autosomal_joint_analysis/allelic_ratios_autosome_chr11.pdf", width = 20, height = 20)
# 
# (counts_active(data_pca) / (counts_active(data_pca) + counts_inactive(data_pca))) %>%
#   add_column("chr" = as.character(seqnames(rowRanges(data_pca)))) %>%
#   add_column("pos" = start(rowRanges(data_pca))) %>%
...
#   dplyr::filter(chr == 13) %>%
#   ggplot(aes(x = pos, y = value, col = Condition)) + geom_point(size = .1) + facet_wrap(~Clone+Replicate) + ylim(c(0, 1)) + geom_hline(yintercept = .5)
# ggsave("./Plots/FigsX_autosomal_joint_analysis/allelic_ratios_autosome_chr13.pdf", width = 20, height = 20)
# 
# (counts_active(data_pca) / (counts_active(data_pca) + counts_inactive(data_pca))) %>%
#   add_column("chr" = as.character(seqnames(rowRanges(data_pca)))) %>%
#   add_column("pos" = start(rowRanges(data_pca))) %>%
...
# data_pca <- runPCA(data_pca)
# rotations_pca <- attr(calculatePCA(data_pca), "rotation")
# saveRDS(rotations_pca, "./ProcessedData/pca_npcs.rds")
# saveRDS(data_pca, "./ProcessedData/for_joint_pca.rds")
# 
# pdf("./Plots/FigsX_autosomal_joint_analysis/cor_map.pdf")
# pheatmap::pheatmap(cor(logcounts(data_pca[rowMeans(counts(data_pca)) > 1000, ])))
# dev.off()
# 
# data_pca <- data_with_autosomes[,data_with_autosomes$Condition %in% c("Aux_0_Dox_0_WO_0_WOAuxNO", "Aux_0_Dox_7_WO_0_WOAuxNO")]
# data_pca <- data_pca[, data_pca$Clone != "E6"]
# data_pca <- computeSumFactors(data_pca)
# data_pca <- logNormCounts(data_pca)
# data_pca <- runPCA(data_pca)
# rotations_pca <- attr(calculatePCA(data_pca), "rotation")
# saveRDS(rotations_pca, "./ProcessedData/pca_cl30s.rds")
# 
# data.frame(
#   PC1 = reducedDims(data_pca)[["PCA"]][, 1],
#   PC2 = reducedDims(data_pca)[["PCA"]][, 2],
#   Condition = data_pca$ConditionClean,
#   Clone = data_pca$Clone
# ) %>% ggplot(aes(x = PC1, y = PC2, col = Condition, shape = Clone)) + geom_point(size = 4)
# ggsave('./Plots/FigsX_autosomal_joint_analysis/pca_npcs_cl30.pdf')
# 
# data.frame(
#   Expression = as.numeric(logcounts(data_pca)["Pax6", ]),
#   Condition = data_pca$ConditionClean,
#   Clone = data_pca$Clone
# ) %>% ggplot(aes(x = Clone, y = Expression, col = Condition)) + geom_point(position = position_dodge(width = .5))
...
# Now read our pre-processed dataset
# data_here <- data_with_autosomes[,data_with_autosomes$Clone %in% c("CL30.7", "CL31.16", "CL31", "CL30")]
# 
# data_here <- data_here[seqnames(rowRanges(data_here)) != "X", ]
# 
# data_here <- computeSumFactors(data_here)
# data_here <- logNormCounts(data_here)
# data_here <- runPCA(data_here)
# 
# data.frame(
#   PC1 = reducedDims(data_here)[["PCA"]][, 1],
#   PC2 = reducedDims(data_here)[["PCA"]][, 2],
#   Condition = data_here$ConditionClean,
#   Clone = data_here$Clone
# ) %>% ggplot(aes(x = PC1, y = PC2, col = Condition, shape = Clone)) + geom_point(size = 4)
# ggsave("./Plots/FigsX_autosomal_joint_analysis/all_cl30_pca.pdf", width = 20, height = 20)
...
#                                               "Aux (2d)", "Aux (5d)", "Aux (9d)", 
#                                               "Dox (3d), Aux (5d)", "Dox (7d), Aux (9d)")]
# data_here <- data_here[,data_here$Clone %in% c("CL30.7", "CL31.16", "CL30", "CL31")]
# data_here <- data_here[,data_here$Condition %in% c("Aux_0_Dox_0_WO_0_WOAuxNO", "Aux_0_Dox_3_WO_0_WOAuxNO", "Aux_0_Dox_7_WO_0_WOAuxNO", 
#                                                    "Aux_2_Dox_0_WO_0_WOAuxNO", "Aux_5_Dox_3_WO_0_WOAuxNO", "Aux_9_Dox_7_WO_0_WOAuxNO")]
# 
# levels_use <- c("Control", "Dox (3d)", "Dox (3d), Aux (5d)", "Dox (7d)", "Dox (7d), Aux (9d)")
# 
# # Define clones / subclones
...
# escapees_per_clone <- lapply(escapees_per_clone, function(x){x[!x %in% c("Xist", "Tsix")]})
# escapees_per_clone <- lapply(escapees_per_clone, function(x){x[!is.na(x)]})
# names(escapees_per_clone) <- c("CL30", "CL31", "CL31.16", "CL30.7")
# 
# ratios %>%
#   t() %>% data.frame() %>%
#   add_column(Dox = data_here$ndDox) %>%
#   add_column(Aux = pmax(as.numeric(data_here$ndAux) - 2, 0)) %>%
#   add_column(Clone = data_here$SubClone) %>%
#   add_column(Condition = data_here$ConditionClean) %>%
#   pivot_longer(-c(Dox, Aux, Clone, Condition)) %>%
#   saveRDS("./ProcessedData/allelic_ratios_cl30s_test_replicate_resolved.rds")
# 
# # new plot into timepoints
# ratios %>%
#   t() %>% data.frame() %>%
...
# # ggplot2::ggsave("./Plots/FigS2/SPEN_escape_by_sample.pdf")
# 
# cl30_escapees <- intersect(escapees_per_clone$CL30, escapees_per_clone$CL30.7)
# cl31_escapees <- intersect(escapees_per_clone$CL31, escapees_per_clone$CL31.16)
# 
# ratios[cl30_escapees, ] %>%
#   t() %>% data.frame(check.names = F) %>%
#   add_column("Condition" = data_here$ConditionClean) %>%
...
# ### 
# p1 <- to_plot %>%
#   dplyr::filter(Clone %in% c("CL30", "CL30.7")) %>%
#   mutate(Clone = gsub("\\.[0-9]*$", "", Clone)) %>%
#   group_by(Condition, name, position, TimePoint) %>%
#   summarize(value = mean(value)) %>%
#   ggplot(aes(reorder(name, position), y = Condition, fill = value, col = value)) + geom_tile(width = 0.9) +
#     scale_fill_gradient2(midpoint = 0.5, mid = "#117733", low = "#BEBEBE", high = "black", limits = c(0, 1), na.value = "white") + 
#     scale_colour_gradient2(midpoint = 0.5, mid = "#117733", low = "#BEBEBE", high = "black", limits = c(0, 1), na.value = "white") + 
...
# 
# p1 / p2 + plot_layout(heights = c(4, 0.2))
# #ggplot2::ggsave("./Plots/Fig2/heatmap_spen_only_escapees_by_position_cl30_replicate_resolved.pdf", width = 13, height = 3.5)
# ggplot2::ggsave("./Plots/FigS2/heatmap_spen_only_escapees_by_position_cl30_replicate_merged.pdf", width = 13, height = 3.5)
# 
# 
# #### CL31
# ratios[cl31_escapees, ] %>%
...
# 
# p1 <- to_plot %>%
#   dplyr::filter(Clone %in% c("CL31", "CL31.16")) %>%
#   mutate(Clone = gsub("\\.[0-9]*$", "", Clone)) %>%
#   group_by(Condition, name, position, TimePoint) %>%
#   summarize(value = mean(value)) %>%
#   ggplot(aes(reorder(name, position), y = Condition, fill = value, col = value)) + geom_tile(width = 0.9) +
...
# 
# p1 / p2 + plot_layout(heights = c(4, 0.2))
# ggplot2::ggsave("./Plots/FigS2/heatmap_spen_only_escapees_by_position_cl31_replicate_merged.pdf", width = 13, height = 3.5)
